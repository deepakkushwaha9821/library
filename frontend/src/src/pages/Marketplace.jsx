...
const Marketplace = ({ onSelectBook }) => {
  ...
  const fetchUserLibrary = async () => {
    try {
      ...
      const owned = (res.data.ownedBooks || []).map(b => b.book.id || b.book._id);
      const rented = (res.data.rentedBooks || []).filter(b => !b.isExpired).map(b => b.book.id || b.book._id);
          ...
          {books.map((book) => {
            ...
            return (
              <BookCard
                ...
                isOwned={userOwnedBookIds.includes(bId)}
                isRented={userRentedBookIds.includes(bId)}
              />
            );
          })}
        </div>
